...
BEGIN
  ...
  -- Elsõ Tanács kérés utáni: Észrevettem hogy még lehetne több táblát historizációval ellátni.
...
CREATE OR REPLACE PACKAGE util_pkg IS
  ...
  --------------------------------------------------------------------
  -- Következõ task_key generálása projekt szinten
  --------------------------------------------------------------------
...
CREATE OR REPLACE PACKAGE BODY board_mgmt_pkg IS
  ...
  BEGIN
    ------------------------------------------------------------------
    -- Projekt létezésének ellenõrzése
    ------------------------------------------------------------------
    SELECT COUNT(*)
      INTO l_project_cnt
      FROM app_project
  ...
  EXCEPTION
    WHEN dup_val_on_index THEN
      raise_application_error(-20111,
                              'create_board_prc: ütközés az egyedi constrainten (valószínûleg (project_id, board_name) vagy (project_id, position)). ' ||
  ...
  BEGIN
    ------------------------------------------------------------------
    -- Ellenõrzés: a board tényleg ehhez a projekthez tartozik?
    ------------------------------------------------------------------
    SELECT COUNT(*)
      INTO l_cnt
      FROM board
...
CREATE OR REPLACE PACKAGE BODY column_mgmt_pkg IS
  ...
  BEGIN
    ...
    ------------------------------------------------------------------
    -- 1. A rendezendõ oszlop boardja és aktuális pozíciója
    ------------------------------------------------------------------
    ...
    FOR i IN 1 .. l_cnt LOOP
      -- Mozgatott oszlopot kihagyjuk az eredeti sorrendbõl
      IF l_all_ids(i) = p_column_id THEN
        CONTINUE;
      END IF;

      -- Ha a célpozícióhoz értünk, elõbb a mozgatott oszlop megy
    ...
    --    (így azonnal megszûnik a 1..N tartományban minden ütközés)
    ------------------------------------------------------------------
    UPDATE column_def
...
CREATE OR REPLACE PACKAGE BODY task_mgmt_pkg IS
  ...
  --  4) Az új sorrendnek megfelelõen 1..N-re újraszámozzuk a position-t.
  --------------------------------------------------------------------
  PROCEDURE reorder_task_in_column_prc(p_task_id      IN task.id%TYPE
                                      ,p_new_position IN task.position%TYPE) IS
  ...
  BEGIN
    ...
    FOR i IN 1 .. l_cnt
    LOOP
      -- A mozgatott taskot kihagyjuk az eredeti sorrendbõl
      IF l_all_ids(i) = p_task_id
      THEN
        CONTINUE;
      END IF;
    
      -- Ha elértük a célpozíciót, elõbb a mozgatott task kerül be
...
EXCEPTION
  --------------------------------------------------------------------
  -- Az ismert, domain-szintû hibákat NEM dobjuk tovább,
  -- mert az eredeti INSERT/UPDATE ettõl még lehet sikeres.
...
CREATE OR REPLACE PACKAGE BODY task_autofill_pkg IS
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        -- Közben esetleg eltûnt a backlog task ez sem igazi hiba
        g_autofill_running := FALSE;
        RETURN;
    END;
