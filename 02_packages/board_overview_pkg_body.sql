CREATE OR REPLACE PACKAGE BODY board_overview_pkg IS
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN no_data_found THEN
        err_log_pkg.log_error(p_module_name    => 'BOARD_OVERVIEW',
    ...
    LOOP
      ...
      FOR t_rec IN (SELECT t.id
                          ...
                          ,
                           ...
                           -- ASSIGNEES
                           (SELECT listagg(u2.display_name, ', ') within GROUP(ORDER BY u2.display_name)
                          ...
                          ,
                           ...
                           (SELECT listagg(a.attachment_type, ', ') within GROUP(ORDER BY a.attachment_type)
                              FROM attachment a
      ...
      LOOP
        l_tasks.extend;
        l_tasks(l_tasks.last) := ty_task_overview(t_rec.id,
                                                  t_rec.task_key,
                                                  t_rec.title,
                                                  t_rec.description,
                                                  t_rec.status_code,
      ...
      ----------------------------------------------------------------
      l_columns.extend;
      l_columns(l_columns.last) := ty_column_overview(c_rec.id,
                                                      c_rec.column_name,
                                                      c_rec.wip_limit,
                                                      c_rec.status_code,
                                                      c_rec.status_name,
    ...
    ----------------------------------------------------------------
    RETURN ty_board_overview(p_board_id,
                             l_board_name,
                             p_sprint_id,
                             l_sprint_name,
                             l_columns);
  ...
  EXCEPTION
    WHEN pkg_exceptions.board_overview_board_not_found
         OR pkg_exceptions.board_overview_sprint_mismatch THEN
      RAISE;
