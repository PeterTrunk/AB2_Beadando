CREATE OR REPLACE PACKAGE BODY task_mgmt_pkg IS
  ...
  EXCEPTION
    WHEN DUP_VAL_ON_INDEX THEN
      ...
      err_log_pkg.log_error(
        ...
        p_context        => 'project_id=' || p_project_id ||
  ...
  --------------------------------------------------------------------
  PROCEDURE assign_user_to_task_prc(
    p_task_id IN task_assignment.task_id%TYPE,
  ...
  BEGIN
    ...
    ------------------------------------------------------------------
    IF p_new_position IS NOT NULL AND p_new_position < 1 THEN
      err_log_pkg.log_error(
        p_module_name    => 'TASK',
    ...
    IF l_new_board_id <> l_old_board_id THEN
      err_log_pkg.log_error(
        ...
        p_context        => 'task_id=' || p_task_id ||
                            '; old_board_id=' || l_old_board_id ||
                            '; new_board_id=' || l_new_board_id,
        p_api            => NULL
      );
    ...
    IF l_wip_limit IS NOT NULL AND l_wip_limit > 0 THEN
      ...
      IF l_active_count >= l_wip_limit THEN
        err_log_pkg.log_error(
          ...
          p_context        => 'task_id=' || p_task_id ||
                              '; new_column_id=' || p_new_column_id ||
                              '; wip_limit=' || l_wip_limit ||
                              '; active_count=' || l_active_count,
          p_api            => NULL
        );
    ...
    -- p_actor_id: késõbb használható activity loghoz
