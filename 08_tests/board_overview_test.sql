DECLARE
  ...
  l_found_closed_user BOOLEAN := FALSE; -- „Alap felhasználók felvétele”-task (ez már lezárt)
...
BEGIN
  ------------------------------------------------------------------
  -- Teszt: BOARD_OVERVIEW + AGGREGÁCIÓK
  -- Kritériumok: 
  ------------------------------------------------------------------  
  -- Oszlopszám konzisztencia
  --   - A függvény által visszaadott oszlopok száma
  --   = a COLUMN_DEF táblában lévő oszlopok száma az adott boardhoz.

  -- Task darabszám oszloponként
  --   - Minden oszlopnál a visszaadott tasks.COUNT
  --   = TASK táblában lévő, adott board + column + sprint + (closed_at IS NULL) rekordok száma.

  -- "DB séma kialakítása" task (TODO oszlop, aktív)
  --   - Szerepel az overview-ben.
  --   - attachment_count >= 1.
  --   - attachment_types tartalmazza a 'DESIGN' típust.
  --   - labels_text tartalmazza a 'backend' címkét.
  --   - has_commit = 'N' és has_pr = 'N' (nincs git link).

  -- "Historizáció implementálása" task (IN_PROGRESS, aktív)
  --   - Szerepel az overview-ben.
  --   - labels_text tartalmazza a 'backend' címkét.
  --   - has_commit = 'Y' (van commit_link).
  --   - has_pr = 'Y' (van pr_link).
  
  -- "Alap felhasználók felvétele" task (DONE, lezárt)
  --   - Nem jelenhet meg az overview-ben (closed_at IS NOT NULL → szűrjük).
